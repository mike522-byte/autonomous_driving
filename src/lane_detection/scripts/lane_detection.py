...
class camera:
    def __init__(self):
        ...
        src_points = np.array([[360,360], [920,360], [1280,720], [0,720]], dtype="float32")
        dst_points = np.array([[0, 0], [1280, 0], [1280, 720], [0, 720]], dtype="float32")
    ...
    def spin(self):
	    ...
	    print('1')
            ...
            #img = cv2.resize(img,(int(0.125*width),int(0.125*height)),interpolation=cv2.INTER_CUBIC)          
            #gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
            hlow = np.array([30,70,70])
            hhigh = np.array([34,255,255])
            origin_thr = cv2.inRange(gray_img,lowerb=hlow,upperb=hhigh)
            kernel = np.ones((3,3), np.uint8)

            origin_thr = np.zeros_like(gray_img)
            origin_thr[(gray_img >= 240)&(gray_img <= 250)] = 255

            origin_thr = cv2.erode(origin_thr, kernel, iterations=2)
            #origin_thr = cv2.dilate(origin_thr, kernel, iterations=1)
            # toushi transform
	    cv2.imshow('img',img)
